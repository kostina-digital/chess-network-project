      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {steps.map(({ step, title, body }) => (
            <article
              key={step}
              className="relative rounded-xl border border-border bg-card p-6 shadow-sm flex flex-col gap-2 ">
              <h3 className="h3-style m-0"> {step}. <b>{title}</b></h3>
              <p className="p-style">{body}</p>
            </article>
          ))}
        </div>
      </div>
